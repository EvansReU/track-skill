...
- 搜索当前项目中和 `xxxx` 有关的历史记录。
- 返回关联问题、决策、产物和上下文。

此外，已建档项目会自动 Track 重要轮次。自动 Track 默认本地执行，不调用 AI，不全文召回。
...
当 Codex 本轮已经自动执行 `track this` 或 `track auto` 并保存当前对话时，回复末尾应显示：
...
Track has only two core explicit forms:

```text
Track
```

- First use: create a Track record for the current project and run lightweight backfill.
- In an already tracked project: force-save the current turn.

```text
Track xxxx
```

- Search the current project for records related to `xxxx`.
- Return related questions, decisions, artifacts, and context.

In addition, already tracked projects automatically Track important turns. Auto Track is local by default, does not call AI, and does not recall full history.
...
These pause and resume auto Track for the current project.
...
When Codex automatically runs `track this` or `track auto` and saves the current conversation in a turn, its reply should end with:
